...
public:
    ...
    struct LatticePlannerParams : public LocalPlannerInterface::LocalPlannerParamsInterface
    {
        ...
        double sample_l_range = 2.0;          // 横向采样范围 (±m)
        double sample_l_step = 0.5;           // 横向采样步长 (m)
        double sample_T_min = 3.0;             // 最小规划时间 (s)
        double sample_T_max = 8.0;             // 最大规划时间 (s)
...
public:
    ...
    // 初始化
    void InitParams(const LocalPlannerParamsInterface & params);
