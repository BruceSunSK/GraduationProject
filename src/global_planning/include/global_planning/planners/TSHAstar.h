...
///     总体使用而言，B样条更快，更平滑。
/// [6] 使用数值优化平滑路径
///     1. 对曲线平滑后的结果进行三次样条插值，可以按照任意散度ds进行采样且可得到任意阶导数。
///     2. 然后利用二次规划QP对离散后的点进行优化。目前具体方法和Apollo的FesPosSmoother一样。
///        2.1 代价函数：1.平滑代价。 2.长度代价。 3.偏离代价
///        2.2 约束：起点终点必定与原始相同，其余点有边界范围限制。
///        2.3 此过程不考虑障碍物。因为原始点已经相对在路中间了，所以在小范围内不考虑障碍物基本没问题；
///            而且这个路径只是后面【第二大步】的参考路径，更注重的是平滑效果，而避障效果将在【第二大步】中详细计算。
///
