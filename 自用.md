...
- GlobalPlanning中的特殊使用：
  - 在GlobalPlanning的test_global_planning.cpp中，跳过了grid_cost_map节点，直接在该节点中加载了png作为cv::Mat，并直接传入planner中。
...
## 决策模块
在状态机中，我只对部分状态的参数进行了设置，因为只有这些状态会在实验中被用到。
- ignore到其他状态的转移、overtake保持自己状态。体现在多帧动态实验中。
- follow到其他状态的转移。体现在单帧同向实验中。
- yield到其他状态的转移。体现在单帧交叉和避让实验中。
## 可视化
### test_local_planning
- 仿真：carla；实车：XG
- 场景：static静态场景；dynamic：动态会车场景
- xy图太扁平？搜equal改成auto，但这样车辆外形就不是矩形了。搜330改改y轴数据范围，使得看上去合理就行；辆车离得近点，这样图就不长了。
- xy图障碍物位置不对？记得该读取的障碍物yaml路径。
- xy图或其他图点太密集？修改降采样参数，可以整体改，也可以单独改。
...
轨迹点的速度使用pose.position.z代替。（原因：不想引入新的话题导致时间差/不想自定义消息类型）

# 6. simulation
自己通过代码模拟仿真环境，引入了噪声，目前为100Hz。在此包中，线速度v可以为负数。 
需要设置车辆的初始位姿，记得需要和全局参考线进行对齐。
- 在测试代码中，全局参考线的起点终点是手动设置的；对应地需要正确设置simulation中的车辆起点位姿。
- 在正式运行代码中，全局参考线会自定订阅当前位姿作为起点，因此只需要合理设置simulation的车辆起点位姿即可。
